...
export default function FeaturedWorks() {
  ...
  // All work images in order — used to open gallery starting at the right position
  const allWorkImages = works.map((w) => w.image);
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12 lg:px-16">
        ...
        <div className="flex flex-wrap gap-4 mb-16">
          {categories.map((cat) => (
            <button
              ...
              className={`px-6 py-2 rounded-full font-sans text-sm font-light tracking-elegant transition-all duration-300 touch-manipulation border ${
                activeFilter === cat
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {filteredWorks.map((work) => {
            // Find index within the full (unfiltered) works list so gallery navigation is consistent
            const globalIndex = allWorkImages.indexOf(work.image);
            return (
              <div
                key={work.id}
                onClick={() => openGallery(allWorkImages, globalIndex)}
